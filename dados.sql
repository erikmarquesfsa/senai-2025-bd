...
select produto, 
preco_unitario*quantidade as vl_vendido  
from vendas;

-- quais os produtos vendidos por ana
select produto from vendas 
where vendedor = 'Ana';

-- selecione todos os produtos da categoria eletrônicos
select produto from vendas
where categoria = 'Eletrônico'

-- Conte quantos registros existe na tabela vendas
select count(produto) from vendas;

-- Quanto foi vendido em cada categoria
select categoria,
sum(preco_unitario*quantidade) as vl_venda 
from vendas
group by categoria
...
-- quanto foi a média vendida por dia
select data_venda,categoria,
SUM(preco_unitario*quantidade) as vl_vendido,
avg(preco_unitario*quantidade) as md_vendida
from vendas
group by categoria,data_venda;
